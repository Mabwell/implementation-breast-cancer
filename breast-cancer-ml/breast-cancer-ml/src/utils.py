...
def main():
    ...
    # In sklearn's dataset target encoding, verify indices
    # Find index of 'malignant' in target_names and map accordingly
    malignant_idx = int(np.where(target_names == 'malignant')[0][0])
    # Create y where malignant -> 1, benign -> 0
    ...
    # liblinear solver is suitable for small datasets and binary classification
...
if __name__ ==def save_model(model, filename):
    import joblib
